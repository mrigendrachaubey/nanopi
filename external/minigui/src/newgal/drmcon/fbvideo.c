...
VideoBootStrap DRMCON_bootstrap = {
    "drmcon", "Linux DRM Console",
...
static int FB_VideoInit(_THIS, GAL_PixelFormat *vformat)
{
    ...
    if (vformat->BitsPerPixel == 32) {
        for ( i=0; i<8; ++i ) {
            vformat->Rmask <<= 1;
            vformat->Rmask |= (0x00000001<<0);
        }

        for ( i=0; i<8; ++i ) {
            vformat->Gmask <<= 1;
            vformat->Gmask |= (0x00000001<<8);
        }

        for ( i=0; i<8; ++i ) {
            vformat->Bmask <<= 1;
            vformat->Bmask |= (0x00000001<<16);
        }

        for ( i=0; i<0; ++i ) {
            vformat->Amask <<= 1;
            vformat->Amask |= (0x00000001<<24);
        }
    } else {
        ...
        for ( i=0; i<6; ++i ) {
            vformat->Gmask <<= 1;
            vformat->Gmask |= (0x00000001<<5);
        }
        for ( i=0; i<7; ++i ) {
            vformat->Bmask <<= 1;
            vformat->Bmask |= (0x00000001<<11);
        }
    }
                ...
                int width, int height, int bpp, Uint32 flags)
{
    ...
    if (!GAL_ReallocFormat(current, bpp,
    ...
    current->w = w;
    current->h = h;
    current->pitch = bo.pitch;
    current->pixels = bo.ptr;

    /* Set up the information for hardware surfaces */
    surfaces_mem = (char *)current->pixels;
    surfaces_len = bo.size;
