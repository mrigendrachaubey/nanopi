...
class CamHwItf : public CamApiItf, virtual public enable_shared_from_this<CamHwItf> {
 public:
  ...
  static struct HAL_ISP_Reboot_Req_s mApiReboot;
  ...
  //system info
  virtual int getSysInfo(struct HAL_ISP_Sys_Info_s& info);
  //sensor
  virtual int getSensorInfo(struct HAL_ISP_Sensor_Info_s& info);
  virtual int setSensorExp(struct HAL_ISP_Sensor_Exposure_s& exp);
