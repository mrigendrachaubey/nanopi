...
int CamCifDevHwItf::setWhiteBalance(HAL_WB_MODE wbMode) {
...
int CamCifDevHwItf::setAeBias(int aeBias) {
...
int CamCifDevHwItf::setAeMode(enum HAL_AE_OPERATION_MODE aeMode) {
  int ret = 0;
  int mode = -1;
  switch (aeMode) {
...
int CamCifDevHwItf::getSupportedAeModes(vector<enum HAL_AE_OPERATION_MODE>& aeModes) {
  UNUSED_PARAM(aeModes);
  //TODO: may be got from XML config file
  return -1;
}
int CamCifDevHwItf::getAeMode(enum HAL_AE_OPERATION_MODE& aeMode) {
  aeMode = mCurAeMode;
  return 0;
}

int CamCifDevHwItf::getAeBias(int& curAeBias) {
  curAeBias = mCurAeBias;
  return 0;
}

int CamCifDevHwItf::getSupportedBiasRange(HAL_RANGES_t& range) {
  //UNUSED_PARAM(range);
...
int CamCifDevHwItf::setAbsExp(int exposure) {
#if 0
  int ret = 0;
  struct v4l2_ext_control ctrls[1];
...
int CamCifDevHwItf::getAbsExp(int& curExposure) {
  curExposure = mCurAbsExp;
  return 0;
}

int CamCifDevHwItf::getSupportedExpMeterModes(vector<enum HAL_AE_METERING_MODE> modes) {
  UNUSED_PARAM(modes);
  //TODO : may be got from xml file
  return -1;
}
int CamCifDevHwItf::setExposureMeterMode(enum HAL_AE_METERING_MODE aeMeterMode) {
  int ret = 0;
  int mode = -1;
  switch (aeMeterMode) {
...
int CamCifDevHwItf::getExposureMeterMode(enum HAL_AE_METERING_MODE& aeMeterMode) {
  aeMeterMode = mCurAeMeterMode;
  return 0;
}
int CamCifDevHwItf::setFps(HAL_FPS_INFO_t fps) {
  int ret = 0;
...
int CamCifDevHwItf::enableAe(bool aeEnable) {
  UNUSED_PARAM(aeEnable);
  //do nothing
  return 0;
}

int CamCifDevHwItf::getSupportedAntiBandModes(vector<enum HAL_AE_FLK_MODE>& flkModes) {
...
int CamCifDevHwItf::setAntiBandMode(enum HAL_AE_FLK_MODE flkMode) {
  int mode = -1, ret = 0;
  if (mFlkMode == flkMode)
    return 0;
  switch (flkMode) {
...
//AWB
int CamCifDevHwItf::getSupportedWbModes(vector<HAL_WB_MODE>& modes) {
  //TODO: may be got from xml file
  //UNUSED_PARAM(modes);
...
int CamCifDevHwItf::getWhiteBalanceMode(HAL_WB_MODE& wbMode) {
  wbMode = m_wb_mode;
  return 0;
}
int CamCifDevHwItf::enableAwb(bool awbEnable) {
...
//AF
int CamCifDevHwItf::setFocusPos(int position) {
...
int CamCifDevHwItf::getFocusPos(int& position) {
...
int CamCifDevHwItf::getSupportedFocusModes(vector<enum HAL_AF_MODE> fcModes) {
  //TODO:may be got from xml file
  UNUSED_PARAM(fcModes);
  return -1;
}
int CamCifDevHwItf::setFocusMode(enum HAL_AF_MODE fcMode) {
  int ret = 0;
  int mode = -1;
  switch (fcMode) {
...
int CamCifDevHwItf::getFocusMode(enum HAL_AF_MODE& fcMode) {
  fcMode = mAfMode;
  return 0;
}

int CamCifDevHwItf::getAfStatus(enum HAL_AF_STATUS& afStatus) {
  int ret = 0;
  struct v4l2_ext_control ctrls[1];
...
int CamCifDevHwItf::trigggerAf(bool trigger) {
  int ret = 0;
...
int CamCifDevHwItf::enableAf(bool afEnable) {
  UNUSED_PARAM(afEnable);
  return -1;
}

int CamCifDevHwItf::getSupported3ALocks(vector<enum HAL_3A_LOCKS>& locks) {
  UNUSED_PARAM(locks);
  //TODO:may be got from xml file
  return -1;
}
int CamCifDevHwItf::set3ALocks(int locks) {
  int ret = 0;
...
int CamCifDevHwItf::get3ALocks(int& curLocks) {
  curLocks = m3ALocks;
  return 0;
}

bool CamCifDevHwItf::enumSupportedFmts(vector<RK_FRMAE_FORMAT>& frmFmts) {
...
bool CamCifDevHwItf::enumSupportedSizes(RK_FRMAE_FORMAT frmFmt, vector<frm_size_t>& frmSizes) {
  mSpDev->enumFrmSize(mSpDev->halFmtToV4l2Fmt(frmFmt), frmSizes);
  return true;
}
bool CamCifDevHwItf::enumSupportedFps(RK_FRMAE_FORMAT frmFmt, frm_size_t frmSize, vector<HAL_FPS_INFO_t>& fpsVec) {
  mSpDev->enumFrmFps(mSpDev->halFmtToV4l2Fmt(frmFmt),
                     frmSize.width, frmSize.height,
                     fpsVec);
  return true;
}
int  CamCifDevHwItf::tryFormat(frm_info_t inFmt, frm_info_t& outFmt) {
...
bool CamCifDevHwItf::enumSensorFmts(vector<frm_info_t>& frmInfos) {
  //enum fmts
  vector<unsigned int> fmtVec;
  mSpDev->enumFormat(fmtVec);
  for (int i = 0; i < fmtVec.size(); i++) {
...
//flash control
int CamCifDevHwItf::getSupportedFlashModes(vector<enum HAL_FLASH_MODE>& flModes) {
  UNUSED_PARAM(flModes);
  //TODO:may be got from xml file
  return -1;
}
int CamCifDevHwItf::setFlashLightMode(enum HAL_FLASH_MODE flMode, int intensity, int timeout) {
  int ret = 0;
...
int CamCifDevHwItf::getFlashLightMode(enum HAL_FLASH_MODE& flMode) {
  flMode = mFlMode;
  return true;
}
//color effect
int CamCifDevHwItf::getSupportedImgEffects(vector<enum HAL_IAMGE_EFFECT>& imgEffs) {
  //TODO:may be got from xml file
  UNUSED_PARAM(imgEffs);
  return -1;
}
int CamCifDevHwItf::setImageEffect(enum HAL_IAMGE_EFFECT image_effect) {
  int ret = 0;
...
int CamCifDevHwItf::getImageEffect(enum HAL_IAMGE_EFFECT& image_effect) {
  image_effect = m_image_effect;
  return true;
}

//scene
int CamCifDevHwItf::getSupportedSceneModes(vector<enum HAL_SCENE_MODE>& sceneModes) {
  //TODO:may be got from xml file
  UNUSED_PARAM(sceneModes);
  return -1;
}
int CamCifDevHwItf::setSceneMode(enum HAL_SCENE_MODE sceneMode) {
  int ret = 0;
...
int CamCifDevHwItf::getSceneMode(enum HAL_SCENE_MODE& sceneMode) {
  sceneMode = mSceneMode;
  return 0;
}

int CamCifDevHwItf::getSupportedISOModes(vector<enum HAL_ISO_MODE>& isoModes) {
  //TODO:may be got from xml file
  UNUSED_PARAM(isoModes);
  return -1;
}
int CamCifDevHwItf::setISOMode(enum HAL_ISO_MODE isoMode, int sens) {
  int ret = 0;
...
int CamCifDevHwItf::getISOMode(enum HAL_ISO_MODE& isoMode) {
  isoMode = mIsoMode;
  return 0;
}

int CamCifDevHwItf::getSupportedZoomRange(HAL_RANGES_t& zoomRange) {
  //TODO:may be got from xml file
  UNUSED_PARAM(zoomRange);
  return -1;
}
int CamCifDevHwItf::setZoom(int zoomVal) {
  int ret = 0;
  struct v4l2_ext_control ctrls[2];
  int ctrNum = 1;
...
int CamCifDevHwItf::getZoom(int& zoomVal) {
  zoomVal = mZoom;
  return 0;
}

//brightness
int CamCifDevHwItf::getSupportedBtRange(HAL_RANGES_t& brightRange) {
  //TODO:may be got from xml file
  UNUSED_PARAM(brightRange);
  return -1;
}
int CamCifDevHwItf::setBrightness(int brightVal) {
...
int CamCifDevHwItf::getBrithtness(int& brightVal) {
  mBrightness = brightVal;
  return 0;
}
//contrast
int CamCifDevHwItf::getSupportedCtRange(HAL_RANGES_t& contrastRange) {
  //TODO:may be got from xml file
  UNUSED_PARAM(contrastRange);
  return -1;
}
int CamCifDevHwItf::setContrast(int contrast) {
...
int CamCifDevHwItf::getContrast(int& contrast) {
  mContrast = contrast;
  return 0;
}
//saturation
int CamCifDevHwItf::getSupportedStRange(HAL_RANGES_t& saturationRange) {
  //TODO:may be got from xml file
  UNUSED_PARAM(saturationRange);
  return -1;
}
int CamCifDevHwItf::setSaturation(int sat) {
...
int CamCifDevHwItf::getSaturation(int& sat) {
  mSaturation = sat;
  return 0;
}
//hue
int CamCifDevHwItf::getSupportedHueRange(HAL_RANGES_t& hueRange) {
  //TODO:may be got from xml file
  UNUSED_PARAM(hueRange);
  return -1;
}
int CamCifDevHwItf::setHue(int hue) {
...
int CamCifDevHwItf::getHue(int& hue) {
  mHue = hue;
  return 0;
}

int CamCifDevHwItf::setFlip(int flip) {
  int ret = 0;
...
int CamCifDevHwItf::getFlip(int& flip) {
  int ret = 0;
  int hFlip = 0;
  int vFlip = 0;
...
int CamCifDevHwItf::queryBusInfo(unsigned char* busInfo) {
  return mSpDev->queryBusInfo(busInfo);
}

//ircut
//-1 means unsupported
int CamCifDevHwItf::isSupportedIrCut() {
  return mSpDev->getCtrl(V4L2_CID_BAND_STOP_FILTER);
}
//>0 means IRCUT is working
int CamCifDevHwItf::getIrCutState() {
  return mSpDev->getCtrl(V4L2_CID_BAND_STOP_FILTER);
}

int CamCifDevHwItf::setIrCutState(int state) {
